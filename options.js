(() => {
  ...
  const SAVE_HISTORY_STORAGE_KEY = "bundleSavedPayloadHistoryV1";
  ...
  const buildHistorySnapshot = () => {
    return {
      ...
      popupBundleId: state.popupBundleId,
      selectedNodeId: state.bundleUi.selectedNodeId,
      focusedNodeId: state.focusedNodeId,
      bundleUiState: {
        expandedTreeIds: cloneValue(state.bundleUi.expandedTreeIds ?? {}),
        searchText: `${state.bundleUi.searchText ?? ""}`,
        searchTextDraft: `${state.bundleUi.searchTextDraft ?? state.bundleUi.searchText ?? ""}`,
        migrationMode: state.bundleUi.migrationMode === true
      },
      collapsedNodes: cloneValue(state.collapsedNodes ?? {}),
  ...
  const applyHistorySnapshot = (snapshot) => {
    ...
    state.popupBundleId = `${snapshot.popupBundleId ?? DEFAULT_POPUP_BUNDLE_ID}`.trim() || DEFAULT_POPUP_BUNDLE_ID;
    ensureRuntimeSpecialRoots(state.roots);
    ensurePopupBundleRoot(state.roots);
    state.bundleUi = {
      ...normalizeBundleUiState(snapshot.bundleUiState ?? state.bundleUi),
      selectedNodeId: `${snapshot.selectedNodeId ?? "__all__"}` || "__all__"
    };
    state.focusedNodeId = snapshot.focusedNodeId ?? null;
    state.collapsedNodes = snapshot.collapsedNodes && typeof snapshot.collapsedNodes === "object"
      ? cloneValue(snapshot.collapsedNodes)
      : {};
    state.activeTab = `${snapshot.activeTab ?? "bundles"}` || "bundles";
  };
  ...
  const getUndoLabel = () => state.history.undoStack[state.history.undoStack.length - 1]?.label ?? "";
  const getRedoLabel = () => state.history.redoStack[state.history.redoStack.length - 1]?.label ?? "";

  const pushHistoryItem = (item) => {
    state.history.undoStack.push(item);
    if (state.history.undoStack.length > MAX_HISTORY_STEPS) {
      state.history.undoStack.splice(0, state.history.undoStack.length - MAX_HISTORY_STEPS);
    }
    state.history.redoStack = [];
  };

  const commitHistoryEntry = (label, beforeSnapshot, afterSnapshot) => {
  ...
  const runTrackedMutation = (label, mutate, { render = true } = {}) => {
    const beforeSnapshot = buildHistorySnapshot();
    const result = mutate();
    const afterSnapshot = buildHistorySnapshot();
    commitHistoryEntry(label, beforeSnapshot, afterSnapshot);
    if (render) {
      renderApp();
    }
    return result;
  ...
  const savePayloadHistory = async (payload) => {
    const nextHistory = [
      {
    ...
    ].slice(0, MAX_SAVED_PAYLOAD_HISTORY);
    state.savedPayloadHistory = nextHistory;
    await storageSet({
      [SAVE_HISTORY_STORAGE_KEY]: nextHistory
    });
  };

  const isMigrationMode = () => state.bundleUi.migrationMode === true;
  ...
  const moveEntryBetweenNodes = (entryId, sourceNodeId, targetNodeId, targetIndex = null) => {
    ...
    const targetEntryIndex = targetIndex === null || targetIndex === undefined
      ? null
      : Math.max(0, Math.min(targetIndex, target.entries.length - 1));
    if (isSameNode && targetEntryIndex === index) {
      return false;
    }
    const [entry] = source.entries.splice(index, 1);
    let normalizedIndex = target.entries.length;
    if (targetEntryIndex !== null) {
      // A row drop means "place after this row". Account for the source row
      // disappearing before we calculate the target's post-removal position.
      normalizedIndex = targetEntryIndex + (isSameNode && index < targetEntryIndex ? 0 : 1);
      normalizedIndex = Math.max(0, Math.min(normalizedIndex, target.entries.length));
    }
    target.entries.splice(normalizedIndex, 0, entry);
    return true;
  };

  const moveEntryWithinNode = (node, entryId, direction) => {
    const index = node?.entries?.findIndex((entry) => entry.id === entryId) ?? -1;
    if (index < 0 || !moveItem(node.entries, index, direction)) {
      return false;
    }
    setTableSortState(node.id, null, null);
    return true;
  };

  const moveEntryAfterTarget = (entryId, sourceNodeId, targetNodeId, targetIndex) => {
    const moved = moveEntryBetweenNodes(entryId, sourceNodeId, targetNodeId, targetIndex);
    if (moved) {
      setTableSortState(targetNodeId, null, null);
    }
    return moved;
  };
  ...
  const beginEditSession = (session) => {
    if (
      ...
      state.history.openEditSession.nodeId === session?.nodeId &&
      state.history.openEditSession.entryId === session?.entryId &&
      state.history.openEditSession.sortKey === session?.sortKey
    ) {
      state.history.openEditSession.composing = session?.composing === true;
      state.editSession = {
        composing: false,
        ...session
      };
    ...
    state.history.openEditSession = {
      ...session,
      composing: session?.composing === true,
  ...
  const commitOpenEditSession = (label = "項目編集") => {
    const openEditSession = state.history.openEditSession;
    state.history.openEditSession = null;
    clearEditSession();
    if (!openEditSession?.beforeSnapshot) {
      return false;
    }
    const afterSnapshot = buildHistorySnapshot();
    return commitHistoryEntry(label, openEditSession.beforeSnapshot, afterSnapshot);
  };
  ...
  const renderEntryTable = (node, effectiveKind) => {
    ...
    node.entries.forEach((entry, entryIndex) => {
      ...
      row.addEventListener("drop", (event) => {
        ...
        event.preventDefault();
        const payload = state.dragPayload;
        if (runTrackedMutation("項目移動", () => {
          return moveEntryAfterTarget(payload.entryId, payload.sourceNodeId, node.id, entryIndex);
        }, { render: false })) {
          setFocusedNode(node.id);
          renderApp();
        }
  ...
  const renderEntryTableV2 = (node, effectiveKind, searchText = "") => {
    ...
    count.textContent = isMigrationMode()
      ? `表示 ${sortedEntries.length} 件 / 選択 ${getSelectedCount(sortedEntries)} 件 / dictionary行き ${routeSummary.dictionary} / token行き ${routeSummary.token} / 要確認 ${routeSummary.review}`
      : `表示 ${sortedEntries.length} 件 / 選択 ${getSelectedCount(sortedEntries)} 件`;
    head.append(title, count);
    ...
    const rubyFromSourceHeader = appendHeaderCell(
      createSortHeaderButton(node.id, "ruby_from_source", "前をルビ化"),
      "check-col"
    );
    rubyFromSourceHeader.title = "変換後にルビが無い場合、変換前をルビとして付与します";
    appendHeaderCell(createSortHeaderButton(node.id, "from", "変換前"), "text-col");
    ...
    sortedEntries.forEach((entry) => {
      ...
      actionTd.appendChild(detailButton);
      const moveUpButton = createButton("↑", "ghost", () => {
        if (runTrackedMutation("項目並べ替え", () => {
          return moveEntryWithinNode(node, entry.id, -1);
        }, { render: false })) {
          renderApp();
        }
      });
      moveUpButton.title = "この項目を上へ移動";
      moveUpButton.disabled = entryIndex <= 0;
      const moveDownButton = createButton("↓", "ghost", () => {
        if (runTrackedMutation("項目並べ替え", () => {
          return moveEntryWithinNode(node, entry.id, 1);
        }, { render: false })) {
          renderApp();
      ...
      moveDownButton.title = "この項目を下へ移動";
      moveDownButton.disabled = entryIndex >= node.entries.length - 1;
      actionTd.append(moveUpButton, moveDownButton);
      actionTd.appendChild(createButton("削除", "danger row-delete", () => {
        runTrackedMutation("項目削除", () => {
          node.entries.splice(entryIndex, 1);
        });
      }));
      ...
      if (tokenEditorAvailable && entry.metaOpen) {
        const detailRow = document.createElement("tr");
        const detailCell = document.createElement("td");
        detailCell.colSpan = isMigrationMode() ? 18 : 17;
  ...
  const renderNodeSection = ({ node, parentChildren, index, depth = 0, isRoot = false, inheritedKind = null, searchText = "" }) => {
    ...
    header.addEventListener("drop", (event) => {
      ...
      if (runTrackedMutation("箱移動", () => {
        return moveNodeBetweenParents(payload.nodeId, parentChildren, index);
      }, { render: false })) {
        setFocusedNode(node.id);
        renderApp();
      }
    ...
    if (isMigrationMode()) {
      ...
      titleWrap.append(dictionaryChip, tokenChip, reviewChip);
    }
  ...
  const renderNodeBulkActionsPanel = ({ node, parentChildren, index, isRoot, effectiveKind }) => {
    ...
    actions.appendChild(createButton("↑", "ghost", () => {
      runTrackedMutation("箱並べ替え", () => {
        moveItem(parentChildren, index, -1);
      });
    }));
    actions.appendChild(createButton("↓", "ghost", () => {
      runTrackedMutation("箱並べ替え", () => {
  ...
  const renderTreeNode = (node, depth = 0) => {
    ...
    row.addEventListener("drop", (event) => {
      ...
      if (runTrackedMutation("箱移動", () => {
        return moveNodeBetweenParents(payload.nodeId, location.parentChildren, location.index);
      }, { render: false })) {
        renderApp();
      }
      state.dragPayload = null;
  ...
  const renderBundles = () => {
...

    {
      ...
      migrationCheckbox.checked = isMigrationMode();
      migrationCheckbox.addEventListener("change", () => {
        state.bundleUi.migrationMode = migrationCheckbox.checked;
        saveBundleUiState();
        renderBundles();
      });
      migrationToggle.append(migrationCheckbox, document.createTextNode("移行モード"));
  ...
  const renderApp = () => {
    ...
    if (undoButton) {
      ...
      undoButton.textContent = getUndoLabel() ? `Undo: ${getUndoLabel()}` : "Undo";
    }
    if (redoButton) {
      redoButton.disabled = state.history.redoStack.length === 0;
      redoButton.textContent = getRedoLabel() ? `Redo: ${getRedoLabel()}` : "Redo";
    }
  ...
  const importSettingsFromText = async (text, fileName) => {
    ...
    runTrackedMutation(`置換読込: ${fileName}`, () => {
  ...
  const initialize = async () => {
    ...
    const [storedPayload, storedDiagnosticUiState, storedBundleUiState, storedSavedPayloadHistory] = await Promise.all([
      ...
      storageGet(SAVE_HISTORY_STORAGE_KEY)
    ]);
    ...
    state.savedPayloadHistory = Array.isArray(storedSavedPayloadHistory)
      ? storedSavedPayloadHistory.slice(0, MAX_SAVED_PAYLOAD_HISTORY)
      : [];
  ...
  runtimeSkipEditableInput.addEventListener("change", () => {
    runTrackedMutation("runtime: editable除外", () => {
  ...
  runtimeGlobalEnabledInput?.addEventListener("change", () => {
    runTrackedMutation("runtime: 全体有効", () => {
